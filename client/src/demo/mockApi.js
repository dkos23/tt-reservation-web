...
async function handleRequests(url, options) {
    console.log("Mock API handling request:", url, options);
    // if (url.includes("base-data")) {
    //     console.log("✅ Mock API intercepted base-data request!");
    //     return {
    //         config: {
    //             timeZone: "Europe/Berlin",
    //             announcement: "MOCK DATA OVERRIDE Tennisplatz Reservierung!",
    //         },
    //         courts: [],
    //         templates: [],
    //     };
    ...
    //TODO
    // console.log("db.config: " + JSON.stringify(db.config,null,2));
    

    for (let s of tryStrings) {
        switch (s) {
            case cn(getBaseDataApi):
                console.log("🔍 Before Returning Base Data:", { ...db });

                const response = {
                    config: {
                        ...db.config,
                        visibleHours: Array.isArray(db.config.visibleHours) ? db.config.visibleHours : [8, 22], // ✅ Ensure it's always an array
                    },
                    courts: db.courts.length ? db.courts : [{ courtId: 1, name: "Default Court" }],
                    templates: db.templates || {},
                };

                console.log("✅ Mock API /api/base-data Response Fixed:", response);
                return response;
...
export function patchFetch() {
    console.log("✅ Mock API fetch() patch applied!");
